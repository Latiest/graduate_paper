...
\begin{table}[H]
    ...
    \label{tab:testcon}
    \begin{tblr}{
        width = \linewidth, % 控制表格宽度为行宽
        colspec = {|X[c]|X[c]|X[c]|X[c]|}, % 设置列格式让文本水平和垂直居中
        row{1} = {font=\bfseries,}, % 第一行字体加粗，背景和字体颜色设置
        hlines, % 每行之间画线
        vlines, % 每列之间画线
        cells = {c}, % 所有单元格内容垂直和水平居中
    }
        {编号} & {用例说明} & {测试步骤} & {预期结果} \\
...
\begin{figure}
    \centering
    \subfloat[预测产气量]{\includegraphics{figure/产气量预测_预测.pdf}%
    \label{pre}}
    \hfil
    \subfloat[模型效果]{\includegraphics{figure/产气量预测_模型效果检验.pdf}%
    \label{model}}
    \caption{产气量预测界面}
    \label{fig:prere}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subfloat[可视化展示]{\includegraphics{figure/开关井预测-图片.pdf}%
    \label{open}}
    \hfil
    \subfloat[分别到集气站的产量]{\includegraphics{figure/开关井推荐-各集气站产量.pdf}%
    \label{station}}
    \hfil
    \subfloat[每日需开关的井]{\includegraphics{figure/开关井推荐-每日变化展示.pdf}%
    \label{dailyopen}}
    \caption{开关井推荐界面}
    \label{fig:openre}
\end{figure}
